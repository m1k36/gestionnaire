...
ViewHistogrammeChien::ViewHistogrammeChien(Data* data, QWidget* widget,QDateEdit*min, QDateEdit* max) :
    data(data),
    widget(widget),
    min(min),
    max(max)
{
    ...
    ViewHistogrammeChien::updateModel();
}


void ViewHistogrammeChien::updateModel()
{
    series->clear();
    axisX->clear();
    ...
    QList<int> boxPerDay = data->getListIntBox(size+1, min->date(), max->date(), "chien");

    QBarSet* set = new QBarSet("Box remplis");
    set->setColor(QColor(70, 70, 255));
